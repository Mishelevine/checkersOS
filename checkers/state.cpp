...
void State::SaveGameResultToFile(int winnerCode) const {
    std::ofstream outFile("game_results.txt", std::ios::app);

    if (outFile.is_open()) {
        char player = (winnerCode == 1) ? 'O' : 'X';
        outFile << "Winner: Player " << player << std::endl;
        outFile << "---------------------------" << std::endl;

        outFile.close();
    }
    else {
        std::cerr << "Unable to open file for saving game results." << std::endl;
    }
}
